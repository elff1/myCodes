/*
* cy_G5_P2_Trie.c - the program for Research Project 2 of Advanced Data Structures and Algorithm Analysis.
* Copyright (C) Zhao Bingqian of Group 5. All rights reserved.
* Purpose: search words in different articles.
* Algorithm: Using Trie to store the inverted file.
...
/*
*The Trie node stored the index word.
*Member 'f' marks it's a word from the root to this node.
*Member 'Next[]' points to its 26 children.
...
WordNodePtr EmptyTree(WordNodePtr t);
WordNodePtr AddWord(WordNodePtr t,char *word,int FileNum,int LineNum);
WordNodePtr FindWord(WordNodePtr p,char *word);
void PrintTree(WordNodePtr t);
...
int main(void){
	...
	*build up the Trie inverted index tree t 
...
WordNodePtr AddWord(WordNodePtr t,char *word,int ArticleNum,int LineNum){
	...
	if(p->f==0){
		p->head=p->tail=AddArticle(ArticleNum,LineNum);
		p->f=1;
	}
	else if(p->tail->order==ArticleNum)
		p->tail->tail=p->tail->tail->next=AddLine(LineNum);
	else
		p->tail=p->tail->next=AddArticle(ArticleNum,LineNum);
	return t;
}
