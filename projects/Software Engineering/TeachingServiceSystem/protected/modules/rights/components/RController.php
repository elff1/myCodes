...
class RController extends CController
{
	...
	*/
	...
	//this function is added by zym1010 to fix the problem that yii-rights-1.3.0 doesn't work in yii-1.1.10.
// 	function filterAccessControl($filterChain) 
// 	{ $filter = new RightsFilter; $filter->allowedActions = $this->allowedActions(); $filter->filter($filterChain); }
	
	
	public function init() //check preferred language
	{
		if(strpos(Yii::app()->request->preferredLanguage,'zh_') !== false) //find the 'zh_'
		{	
