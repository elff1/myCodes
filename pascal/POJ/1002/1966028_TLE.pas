...
var h,p:pt;x,n:longint;s:string;
 ...
 var q,t,r:pt;
 begin
  if h=nil then begin new(p);p^.s:=c;p^.m:=1;p^.n:=nil;h:=p end
   else if c<h^.s then
         begin new(q);q^.s:=c;q^.m:=1;q^.n:=h;h:=q end
    else begin
     t:=nil;r:=h;
     while(r<>nil)and(r^.s<c)do begin t:=r;r:=r^.n end;
     if r=nil then begin new(q);q^.s:=c;q^.m:=1;q^.n:=nil;p^.n:=q;p:=q end
      else if c=r^.s then inc(r^.m) 
       else begin new(q);q^.s:=c;q^.m:=1;q^.n:=r;t^.n:=q end
    end
 end;
 procedure op;
 var t:boolean;q:pt;
...
begin
readln(n);h:=nil;
for x:=1 to n do if ok then jr(ce);
op
end.